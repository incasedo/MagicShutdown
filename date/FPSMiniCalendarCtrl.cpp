...
CFPSMiniCalendarCtrl::CFPSMiniCalendarCtrl()
{
	...
	SetSpecialDaysColor(::GetSysColor(COLOR_BTNFACE));
	...
	SetSpecialDaysCallback(NULL, NULL);
...
CSize CFPSMiniCalendarCtrl::ComputeTotalSize()
{
	...
	//ÐÞÕý²ÎÊý,modified by xiaodingliang 2006-1-13
	size.cy-=120;
	if (m_bShow3dBorder)
...
int CFPSMiniCalendarCtrl::DrawDaysOfWeek(CDC &dc, int iY, int iLeftX, int, int)
{
	...
	dc.DrawText(CString("S"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;

	dc.DrawText(CString("M"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;

	dc.DrawText(CString("T"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;

	dc.DrawText(CString("W"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;

	dc.DrawText(CString("T"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;

	dc.DrawText(CString("F"), 1, CRect(iX, iY+1, iX+m_iIndividualDayWidth,iY+2+m_iDaysOfWeekHeight), DT_SINGLELINE | DT_RIGHT | DT_BOTTOM);
	iX += m_iIndividualDayWidth + 5;
		...
		for (int iDayOfWeek = 1; iDayOfWeek <= 7; iDayOfWeek++)
		{
				...
				(dt < dtStart && iMonthCol == 1 && iMonthRow == 1))
			{
				...
				if (IsDateSelected(dt))
				{
					...
					dc.FillSolidRect(iX-3, iY, m_iIndividualDayWidth+5, m_iDaysHeight+3, cBackColor);
				}

				if (IsSpecialDate(dt))
				{
					pOldFont = dc.SelectObject(m_SpecialDaysFontInfo.m_pFont);
					cTextColor = m_cSpecialDaysColor;
				}
				else
				{
...
BOOL CFPSMiniCalendarCtrl::IsSpecialDate(COleDateTime &dt)
{
	if (m_pfuncCallback)
	{
		try
		{
			if (m_pfuncCallback(dt, m_dwCallbackParm))
				return TRUE;
		}
		catch(...)
		{
...
void CFPSMiniCalendarCtrl::SetSpecialDaysCallback(funcSPECIALDATE pValue, DWORD dwParm)
{
	m_dwCallbackParm = dwParm;
...
void CFPSMiniCalendarCtrl::FireNotifyViewChange()
{
	NMHDR	NotifyArea;

	NotifyArea.code = WM_FPSMINICLNDR_VIEWCHANGE;
	NotifyArea.hwndFrom = m_hWnd;
	NotifyArea.idFrom = ::GetDlgCtrlID(m_hWnd);
