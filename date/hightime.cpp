...
int CHighTime::FindStr(LPCTSTR pszSource, LPCTSTR pszSub, int nStart)
{
	...
	// find first
	LPTSTR pszSearch = (LPTSTR) _tcsstr(pszSource + nStart, pszSub);
...
CString CHighTime::Format(LPCTSTR lpszFormat) const
{
	...
	long double tmp = (long double ) tmHighTimeTemp.nMilli;
	long double tmp1 = (long double ) tmHighTimeTemp.nMicro;
	long double tmp2 = (long double ) tmHighTimeTemp.nNano;

    if (iPos >= 0) {
        szTemp.Format(_T("%01d"),abs(tmp));	// hd+++
        szParse.Replace(_T("%s"),szTemp);
    }
    iPos = szParse.Find(_T("%#s"), 0);
    if (iPos >= 0) {
        szTemp.Format(_T("%d"),abs(tmp));
        szParse.Replace(_T("%#s"),szTemp);
    }
    iPos = szParse.Find(_T("%u"), 0);
    if (iPos >= 0) {
        szTemp.Format(_T("%03d"),abs(tmp1));
        szParse.Replace(_T("%u"),szTemp);
    }
    iPos = szParse.Find(_T("%#u"), 0);
    if (iPos >= 0) {
        szTemp.Format(_T("%d"),abs(tmp1));
        szParse.Replace(_T("%#u"),szTemp);
    }
    iPos = szParse.Find(_T("%n"), 0);
    if (iPos >= 0) {
        szTemp.Format(_T("%03d"),abs(tmp2));
        szParse.Replace(_T("%n"),szTemp);
    }
    iPos = szParse.Find(_T("%#n"), 0);
    if (iPos >= 0) {
        szTemp.Format(_T("%d"),abs(tmp2));
        szParse.Replace(_T("%#n"),szTemp);
    }
