...
BOOL CFPSMiniCalendarListCtrl::OnEraseBkgnd(CDC*) 
{
	return FALSE;
}
...
void CFPSMiniCalendarListCtrl::SetItemsPerPage(int iValue)
{
	...
	double tmp = (double) iValue;
	if (iValue >= 3 && fmod(tmp, 2) != 0)
		m_iItemsPerPage = iValue;
}

void CFPSMiniCalendarListCtrl::SetFont(LPCTSTR lpszFont, int iSize, BOOL bBold, BOOL bItalic, BOOL bUnderline, COLORREF cColor)
{
	ASSERT(iSize > 0);
	ASSERT(iSize <= 72);
	ASSERT(lpszFont);
